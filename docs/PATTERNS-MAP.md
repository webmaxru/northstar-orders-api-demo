...
| `.github/prompts/plan.prompt.md`                  | Research and plan, bound to the read-only `plan` agent.                                                                                                                                                                              |
...
| `scripts/publish-plan.mjs`                 | Opens the plan-first pull request - Learn Option A, a PR containing only the plan - so the phase boundary produces a reviewable artifact rather than a chat message. The read-only planner cannot write it; its `Stop` hook can.                                                               |
...
| `.github/pull_request_template.md`        | Plan (required), evidence, review checklist, limits - the structure the Plan Gate reads.                                            |
| `.github/workflows/plan-gate.yml`         | Required check: the PR description must carry a plan with scope, success criteria and a rollback path.                              |
| `scripts/check-plan.mjs`                  | The gate itself. Checks the description, not whether the template file exists in the repo.                                          |
