...
| Edit the CI workflow | `edit: .github/workflows/ci.yml` | deny - outside the WI-1842 scope |
| Report success | not a tool call | irrelevant; acceptance is decided by evidence, not by a claim |

`tests/unit/tool-authorization.test.ts` asserts all four decisions, so the
